...
2. **Gemini market orders. Decided 2026-09-24.** The owner-run Sandbox check found that Gemini's market buy takes a dollar amount (`MissingTotalSpend`), not a coin quantity, and gives no price protection. The owner chose immediate-or-cancel limit orders capped 1% past a fresh quote, the risk engine's default slippage limit. `GeminiBroker` now sends every `MARKET` request that way; whatever cannot fill within the cap is cancelled at once (#33).
...
Each remaining check is now a one-command script; the plain-language steps are in the [owner-run exchange checks runbook](owner-run-exchange-checks.md).

1. **Gemini Sandbox (Sandbox key):** `python -m probes.gemini_sandbox_lifecycle` runs the lifecycle in 3 and one loop pass for 1 through `GeminiBroker`, and answers the market-order question in open item 2.
2. **Coinbase production, read-only (view-only key):** `python -m probes.coinbase_readonly_reconcile` runs the reconciliation in 4. It refuses an Ed25519 key before any request and reports a 401, which settles open item 3.
3. ~~**Coinbase Advanced Trade sandbox (no key):** capture the fixtures in 5.~~ Done 2026-09-24 (agent-run with the owner's approval; see 5).
